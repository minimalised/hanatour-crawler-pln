...
제공된 여행 상품 데이터를 분석하여, 가독성이 높고 소비자의 구매 욕구를 자극하는 상품명 5개를 생성하세요.
...
[🧱 1단계: 무조건 준수해야 할 상품명 조합 어순 (★필수★)]
최종 생성되는 모든 상품명은 기계가 대충 붙여 넣은 것처럼 만들지 말고, 사람이 읽기 자연스럽도록 아래 구조와 어순을 엄격히 적용하며 단어 사이에 적절한 '띄어쓰기'를 유지하세요.
- 기본 구조: {{필수 출발지 문구}} + {{여행 지역/도시}} + {{일정/기간}} + {{원본의 핵심 고유명사 및 소구점}} + {{콘셉트별 마케팅 키워드}}
- 나쁜 예시 (절대 금지): "공항호텔왕복차량서비스제공 네팔 카트만두" -> 목적지가 뒤에 와서 가독성 붕괴.
- 좋은 예시 (권장): "{departure if departure else '[부산출발]'} 네팔 카트만두 5일 공항 픽업 포함 시내 중심 호텔 숙박"

[💡 2단계: 원본 매력 자산 보존 법칙]
원본 상품명({p['상품명']})에 포함된 구체적인 고유 명사(ex: 특정 골프장 이름, 호텔/리조트명, 항공사 이름, VIP리무진 등)는 상품의 가치를 증명하는 핵심 자산입니다. 
'5성급 숙소', '골프투어' 같은 흔하고 심심한 단어로 뭉뚱그려 바꾸지 말고, 원본의 고유 명사를 상품명에 최대한 그대로 살려 넣으세요.

[⚠️ 3단계: 핵심 제약 가이드라인]
1. 글자 수 제약: 모든 상품명은 공백 포함 최소 32자 ~ 최대 45자 사이로 구성한다. (45자 절대 초과 금지)
2. 어색한 붙여쓰기 금지: 글자 수를 채우기 위해 단어들을 무리하게 다닥다닥 붙여서 가독성을 해치지 마세요. 가독성을 위해 단어 간 공백을 반드시 확보하되, 2칸 연속 공백은 금지합니다.
3. 문장부호 사용 제한: 최종 생성되는 모든 상품명 내부에는 쉼표(,), 느낌표(!), 물결 (~), 플러스(+) 같은 부호나 특수문자를 절대 포함할 수 없다. 범위는 붙임표 대시 기호(ex: 5-6일)로 치환한다. 대괄호[]는 맨 앞 출발지 문구에만 허용한다.
4. 금지어: '신상품', '특가', '대박', '최고' 같은 진부한 단어 및 "주의:", "경고:" 같은 시스템 지시어 삽입을 전면 금지한다.

[🎯 4단계: 콘셉트별 마케팅 지향점]
■ 콘셉트 A (정석 SEO형): [출발지] 지역 + 일정 + 원본의 핵심 고유자산(골프장/호텔 등)을 직관적으로 배열한 정석적이고 깔끔한 상품명.
■ 콘셉트 B (타겟/상황형): [출발지] 지역 + 일정 + 핵심 소구점 뒤에 [부모님효도여행], [가족취향저격], [커플기념일추천] 등 타겟 심리를 저격하는 단어를 자연스럽게 배치.
■ 콘셉트 C (혜택/USP형): [출발지] 지역 + 일정 + 핵심 소구점 뒤에 등급에 맞는 강력한 혜택 명사화 (ex: 전일정식사포함, 노쇼핑노팁보장, 1일자유시간확보).
■ 콘셉트 D (감성/트렌디형): [출발지] 지역 + 일정 + 핵심 소구점 뒤에 트렌디한 감성 키워드 결합 (ex: 요즘뜨는핫플투어, 인생샷명소공략, 낭만야경투어).
■ 콘셉트 E (기본 대안형): 원본 상품명의 원래 정보 순서와 가치를 해치지 않으면서 네이버 쇼핑 규격(글자수, 특수문자 제거)에 맞춰 단정하고 깔끔하게 정돈한 대안.
